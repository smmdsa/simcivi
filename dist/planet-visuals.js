...
import {coordinates,geography,PLANET_RADIUS as R,clamp} from './planet.js';
...
const surfaceFragmentHeader=`uniform vec3 uSun;uniform float uTime;uniform float uWet;uniform float uDetail;varying vec3 vP;varying float vDepth;${noiseGLSL}`;
...
void main(){vec3 n=normalize(vP);float sun=dot(n,uSun),density=cloudsAt(n,uTime),threshold=mix(.69,.53,uWet),cloud=smoothstep(threshold,threshold+.12,density);
...
float grazing=pow(1.-abs(dot(n,normalize(cameraPosition-vP))),2.);float alpha=cloud*(.22+.18*uWet)*uDetail;
...
float shade=(uDetail>.5?smoothstep(.54,.69,cloudsAt(vSurface,uTime)):0.)*max(0.,dot(vSurface,uSun))*uWet*uDetail;
...
vec3 vesperaInstance=(modelMatrix*instanceMatrix*vec4(position,1.)).xyz;vGlow=bioGlow*(1.-smoothstep(-.15,.2,dot(normalize(vesperaInstance),uSun)));`);s.fragmentShader='varying float vGlow;\n'+s.fragmentShader;s.fragmentShader=s.fragmentShader.replace('#include <emissivemap_fragment>','#include <emissivemap_fragment>\ntotalEmissiveRadiance+=vec3(.55,.8,.28)*vGlow;');};mesh.material.customProgramCacheKey=()=> 'vespera-nectar-1';mesh.material.needsUpdate=true;
 ...
 update(sun,time){const w=this.world;this.uniforms.uSun.value.set(sun.x,sun.y,sun.z);this.uniforms.uTime.value=time;this.uniforms.uWet.value=clamp(w.sim.climate.moisture/100+(w.sim.rain>0?.25:0),0,1);this.uniforms.uDetail.value=w.qualityTier===2?0:1;
